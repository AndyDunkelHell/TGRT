...
public abstract class BBHThread : MonoBehaviour { 
	...
	public int threadIdleDelay = 1; //16 for 60fps
    ...
    public float delayBeforeDetect = 3f; //For Arduino Mega must be greater than 1f; for uno it can be 0.5f 
	#endregion
...
//*Without
    ...
    private void OnDataReceived(object sender, SerialDataReceivedEventArgs e)
    {
        try
        {
            // Read as many full lines as are available:
            while (device.BytesToRead > 0)
            {
                string line = device.ReadLine();     // blocks up to ReadTimeout
                GotMessage(line);                    // enqueue / process as before
            }
        }
        catch (TimeoutException)
        {
            // no full line ready—just ignore and wait for next event
        }
        catch (Exception ex)
        {
            Debug.LogError($"[BBHThread] DataReceived handler error: {ex}");
        }
    }
...
//Open Serial port Connection and Board Detection
	public void openConnection() {
        ...
        // --- NEW: event handler for incoming data ---
